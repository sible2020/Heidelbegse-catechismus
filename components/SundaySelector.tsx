...
interface SundaySelectorProps {
  sundays: number[];
  selectedSunday: number;
  onChange: (event: React.ChangeEvent<HTMLSelectElement>) => void;
}

const SundaySelector: React.FC<SundaySelectorProps> = ({ sundays, selectedSunday, onChange }) => {
  return (
    <div className="w-full sm:w-1/2">
      <label htmlFor="sunday-select" className="block text-sm font-medium text-slate-400 mb-2">
        Kies een Zondag
      </label>
      <div className="relative">
        ...
        >
          ...
          {sundays.map((zondag) => (
            <option key={zondag} value={zondag}>
              Zondag {zondag}
            </option>
          ))}
        </select>
